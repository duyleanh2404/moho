...
import { ButtonResendOtp } from './components/button-resend-otp';
    ...
    <div className="wrapper w-full sm:w-[450px] min-h-screen flex flex-col items-center justify-center gap-8 py-12">
      ...
      <div className="space-y-2 text-center">
        ...
        <p className="text-muted-foreground text-sm">
          Chúng tôi đã gửi một mã OTP gồm 6 chữ số đến{' '}
          <span className="text-primary font-medium">{email}</span>.
        </p>
      </div>
      <VerifyEmailForm email={email!} />
      <div className="flex items-center gap-1 text-sm">
        <p className="text-muted-foreground">Chưa nhận được mã xác thực?</p>
        <ButtonResendOtp />
      </div>
    </div>
  );
